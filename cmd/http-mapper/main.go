...
var count = 0
var httpSender *cehttpclienttransformer.HTTPProtocolSender
...
func main() {
	...
	transformer, err = cehttpclienttransformer.NewCeHTTPClientTransformer(config.HTTPTemplate, config.CeTemplate, httpSenderTimeout, config.HTTPJsonBody, config.OnlyPayload, config.Verbose)
	if err != nil {
